...
def sync_exchange_rates(timestamp: int) -> dict:
    ...
    logger.info(f"{LoggingContext.ASSETS} Starting exchange rate sync")
    result = ExchangeRateService.sync_exchange_rates()
    logger.info(
        f"{LoggingContext.ASSETS} Exchange rate sync completed - "
        f"{result['updated']}/{result['total']} rates updated"
    )
    return result
