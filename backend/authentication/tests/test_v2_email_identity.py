...
class V2EmailSerializer(serializers.Serializer):
    email = V2EmailField(required=True)


class V2EmailNormalizerTests(SimpleTestCase):
    def test_normalizes_outer_u0020_and_full_ascii_case(self):
        self.assertEqual(
            normalize_v2_email(" Owner.Name+Tag@EXAMPLE.TEST "),
            "owner.name+tag@example.test",
        )

    def test_preserves_provider_specific_structure(self):
        self.assertEqual(
            normalize_v2_email("First.Last+Tag@Example.COM"),
            "first.last+tag@example.com",
        )

    def test_is_idempotent(self):
        normalized = normalize_v2_email(" Owner@EXAMPLE.COM ")
        self.assertEqual(normalize_v2_email(normalized), normalized)

    def test_accepts_exactly_254_ascii_characters(self):
        address = f"{'a' * 64}@{'b' * 63}.{'c' * 63}.{'d' * 61}"
        self.assertEqual(len(address), 254)
        self.assertEqual(normalize_v2_email(address), address)
    ...
    def test_rejects_more_than_254_raw_characters_before_trimming(self):
        ...
        with self.assertRaises(V2EmailError):
            normalize_v2_email(address)

    def test_rejects_non_string_empty_and_space_only_values(self):
        for value in (None, 1, True, "", "   "):
            with self.subTest(value=value), self.assertRaises(V2EmailError):
                normalize_v2_email(value)

    def test_rejects_nonprintable_ascii_before_normalization(self):
        for character in ("\x00", "\t", "\n", "\r", "\x1f", "\x7f"):
            with self.subTest(character=repr(character)), self.assertRaises(V2EmailError):
                normalize_v2_email(f"owner@example.com{character}")
    ...
    def test_rejects_non_ascii_forms(self):
        for address in (
            ...
            with self.subTest(address=address), self.assertRaises(V2EmailError):
                normalize_v2_email(address)
    ...
    def test_errors_are_fixed_and_do_not_contain_the_input(self):
        ...
        with self.assertRaises(V2EmailError) as raised:
            normalize_v2_email(private_value)
    ...
    def test_destination_expression_uses_lower_trim_and_c_collation(self):
        expression = v2_email_destination_expression()
        self.assertIsInstance(expression, V2EmailDestinationKey)
        trim = expression.source_expressions[0]
        self.assertIsInstance(trim, Trim)
        self.assertEqual(trim.source_expressions, [F("email")])


class V2EmailSerializerTests(SimpleTestCase):
    def test_field_applies_canonicalization(self):
        serializer = V2EmailSerializer(data={"email": " Owner.Name+Tag@EXAMPLE.TEST "})
    ...
    def test_field_rejects_disallowed_raw_input(self):
        ...
        for value in invalid_values:
            with self.subTest(value=repr(value)):
                serializer = V2EmailSerializer(data={"email": value})
    ...
    def test_field_has_the_v2_limit_and_raw_whitespace_contract(self):
        field = V2EmailSerializer().fields["email"]
