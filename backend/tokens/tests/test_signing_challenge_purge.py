...
def a_challenge(wallet, *, expires_in, consumed=False, nonce=1):
    now = timezone.now()
...
class ExpiredChallengesArePurgedTest(TestCase):
    ...
    def setUp(self):
        self.wallet = make_tenant("purge").wallet

    def test_an_unspent_challenge_past_the_retention_window_is_purged(self):
        stale = a_challenge(self.wallet, expires_in=-timedelta(seconds=RETENTION + 60))
    ...
    def test_an_unspent_challenge_inside_the_retention_window_is_kept(self):
        recent = a_challenge(self.wallet, expires_in=-timedelta(seconds=60))
    ...
    def test_a_live_challenge_is_never_purged(self):
        live = a_challenge(self.wallet, expires_in=timedelta(minutes=5))
    ...
    def test_a_consumed_challenge_is_kept_however_old_it_is(self):
        spent = a_challenge(self.wallet, expires_in=-timedelta(days=365), consumed=True)
    ...
    def test_each_pass_deletes_no_more_than_its_batch(self):
        for nonce in range(5):
            a_challenge(self.wallet, expires_in=-timedelta(seconds=RETENTION + 60), nonce=nonce + 1)
    ...
    def test_the_periodic_reports_what_it_removed(self):
        a_challenge(self.wallet, expires_in=-timedelta(seconds=RETENTION + 60))
...
class TheSweepDrainsTheBacklogRatherThanOneBatchTest(TransactionTestCase):
    ...
    def setUp(self):
        self.wallet = make_tenant("sweep").wallet
        self.stale = [
            a_challenge(self.wallet, expires_in=-timedelta(seconds=RETENTION + 60), nonce=n) for n in range(1, 11)
        ]
    ...
    def test_a_spent_challenge_survives_however_many_passes_it_takes(self):
        spent = a_challenge(self.wallet, expires_in=-timedelta(seconds=RETENTION + 60), consumed=True, nonce=99)
