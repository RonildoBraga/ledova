...
class Wallet(BaseModel):
    ...
    def native_balance(self):
        ...
        try:
            native_asset = Asset.objects.filter(
                chain_deployments__chain=self.chain,
            ...
            holding = self.holdings.filter(asset=native_asset).first()
    ...
    def native_market_value(self):
        ...
        try:
            ...
            if not native_asset or not native_asset.current_price:
                return Decimal("0")
            ...
            return holding.quantity * native_asset.current_price
        except Exception:
            return Decimal("0")
    ...
    def market_value(self):
        ...
        total = Decimal("0")
        for holding in self.holdings.select_related("asset").filter(asset__is_active=True):
