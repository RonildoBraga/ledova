...
class LegacyOptimisticDeductionReversalTest(BroadcastTransferGuardTestCase):
    ...
    def send_token(self, amount="1.5", raw=1_500_000, fee=FEE):
    ...
    def send_native(self, amount="0.25", value=25 * 10**16, fee=FEE):
        signed = sign(to=RECIPIENT, value=value, gas=int(Decimal(fee) * 10**9))
        self.tx_hash = Web3.keccak(hexstr=signed).to_0x_hex()
        return transaction_confirmation.create_pending_transaction(
            self.wallet, self.tx_hash, RECIPIENT, Decimal(amount), transaction_fee=Decimal(fee)
        )
    ...
    def test_a_wallet_with_no_native_holding_is_not_given_one_by_a_failed_transfer(self):
        self.hold(self.token, "100")

        self.assertEqual(self.send_token()["status"], "pending")
    ...
    def test_a_transfer_larger_than_the_holding_comes_back_to_where_it_started(self):
    ...
    def test_a_fee_larger_than_the_native_holding_comes_back_to_where_it_started(self):
        self.hold(self.token, "100")
        self.hold(self.native, "5")

        self.assertEqual(self.send_token(fee="500")["status"], "pending")
        self.assertEqual(self.quantity(self.native), Decimal("0"))
        self.fail_transfer(self.tx_hash)
    ...
    def test_a_native_send_larger_than_the_one_holding_comes_back_to_where_it_started(self):
        self.hold(self.native, "0.1")

        self.assertEqual(self.send_native()["status"], "pending")
        self.assertEqual(self.quantity(self.native), Decimal("0"))
        self.fail_transfer(self.tx_hash)

        self.assertEqual(self.quantity(self.native), Decimal("0.1"))

    def test_an_unclamped_transfer_still_comes_back_in_full(self):
        self.hold(self.token, "100")
        self.hold(self.native, "5")
    ...
    def test_the_row_records_what_each_holding_actually_gave_up(self):
        self.hold(self.token, "100")
        self.hold(self.native, "5")
    ...
    def test_a_native_send_records_one_deduction_and_no_separate_fee(self):
        self.hold(self.native, "5")
        self.send_native()
    ...
    def test_a_row_without_a_record_cannot_guess_a_refund_during_a_provider_outage(self):
        self.hold(self.token, "100")
        self.hold(self.native, "5")
        self.send_token()
