// ─────────────────────────────────────────────────────────────────────────────
// Ledova Design Tokens — Single source of truth
//
// Architecture:
//   PALETTE       → Raw color values (private, never import directly)
//   buildColors() → Maps palette → semantic tokens for a given theme
//   DESIGN_TOKENS → Dark theme (default) + shared non-color tokens
//   LIGHT_COLORS  → Light theme color overrides
//
// Consumers:
//   Mobile  → imports DESIGN_TOKENS via theme.ts
//   Web     → CSS generated by packages/scripts/generate-css-tokens.mjs
//             Regenerate: make generate-tokens
// ─────────────────────────────────────────────────────────────────────────────

// ── Tier 1: Primitive Palette ───────────────────────────────────────────────
// Raw color values. These should only be referenced by the semantic layer
// below, never directly in components. Swapping palette values enables theming.
...
const PALETTE = {
  ...
  // Chart UI grays (shared across themes)
  coolGray: {
    400: '#9CA3AF',
    300: '#d1d5db',
    200: '#E5E7EB',
...
// ── Theme Color Builder ─────────────────────────────────────────────────────
// Builds a complete semantic color set from palette mappings.
// Call once per theme; only the palette references change.
...
const LIGHT_COLORS = buildColors({
  surface: {
    base: PALETTE.zinc[50], // Warm off-white (Apple-style, not harsh white)
    raised: PALETTE.white, // Cards pop against the warm base
    tertiary: PALETTE.zinc[100], // Subtle distinction for inputs/wells
    overlay: PALETTE.zinc[200], // Overlays and dropdowns
    disabled: PALETTE.slate[300],
  },
  text: {
    primary: '#1a1a2e', // Near-black with warmth (not pure black)
...
/** Build a shadow token from a single set of values. The `web` CSS string is
 *  computed from the RN parameters — no manual duplication needed. */
...
/** Light theme colors — same shape as DESIGN_TOKENS.colors. */
export { LIGHT_COLORS };
