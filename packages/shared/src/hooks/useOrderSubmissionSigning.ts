...
import type { CreateOrderMessageResponse, Wallet } from '../types';
import type { OrderSubmission } from '../utils/order-submission';

type QrData = { cborHex: string; type: string };
type Encoder = (message: CreateOrderMessageResponse, wallet: Wallet) => QrData | null;
type Signer = (message: CreateOrderMessageResponse, isCurrent: () => boolean) => Promise<string | null>;
...
export function useOrderSubmissionSigning(submission: OrderSubmission, wallet: Wallet | null, encode: Encoder) {
  ...
  const walletReady =
    !!wallet &&
    wallet.uuid === submission.record.walletUuid &&
    wallet.userAccount === submission.record.ownerAccountUuid &&
    wallet.address.toLowerCase() === state.snapshot?.intent.walletAddress.toLowerCase();
  ...
  return { state, view, walletReady, showQr, showSoftware, scan, sign, submitSignature, back, close };
}
